<img align=right src="kubify@2x.png" height="200">
...
There is a [migration path](etcd_migration.md) for switching from self-hosted etcd to static etcd.
...
|Name|Meaning|
|--|--|
|addons|map with addon and their configuration settings (see [addons](addons.md))
...
[Migration](etcd_migration.md) from a self-hosted etcd to a static etcd setup is possible by changing the setting and then starting the recovery from a former backup. The redeploy option must be set to `true` for the etcd migration.
